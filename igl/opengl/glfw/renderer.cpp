...
IGL_INLINE void Renderer::draw( GLFWwindow* window)
{
	...
	for (auto& core : core_list)
	{
		...
		if (coreIndx == right_view)
		{
			
			Eigen::Vector3f v = -scn->GetCameraPosition().cast<float>() - core.camera_translation;
			v[2] = 0;
...
void Renderer::MouseProcessing(int button)
{
	if (scn->isPicked)
	{
		if (button == 1)
		{
			...
			float tmp = core().camera_base_zoom;
			...
			scn->data().TranslateInSystem(scn->MakeTransd().block<3, 3>(0, 0), Eigen::Vector3d(xToMove, 0, 0), true);
			scn->data().TranslateInSystem(scn->MakeTransd().block<3, 3>(0, 0), Eigen::Vector3d(0, yToMove, 0), true);
			scn->WhenTranslate();
		//	scn->data().TranslateInSystem(scn->MakeTransd() * scn->CalcParentsTrans(scn->selected_data_index),Eigen::Vector3d(xToMove, 0, 0),true);
		//	scn->data().TranslateInSystem(scn->MakeTransd() * scn->CalcParentsTrans(scn->selected_data_index),Eigen::Vector3d(0, yToMove, 0),true);
		}
		else
		{
			scn->data().RotateInSystem(scn->data().MakeTransd().block<3,3>(0,0) , Eigen::Vector3d(0, 1, 0), -xrel / 180.0);
			//scn->data().RotateInSystem(scn->MakeTransd() * scn->data().MakeTransd(), Eigen::Vector3d(1, 0, 0), -yrel / 180.0);
			scn->data().MyRotate(Eigen::Vector3d(1, 0, 0), -yrel / 180.0);
//			scn->data().RotateInSystem(scn->MakeTransd() * scn->CalcParentsTrans(scn->selected_data_index) *scn->data().MakeTransd(), Eigen::Vector3d(0, 1, 0), -xrel / 180.0);
//			scn->data().RotateInSystem(scn->MakeTransd() * scn->CalcParentsTrans(scn->selected_data_index) * scn->data().MakeTransd(), Eigen::Vector3d(1,0, 0), -yrel / 180.0);
	...
	else
	{
		...
		{
			scn->RotateInSystem(scn->MakeTransd().block<3, 3>(0, 0), Eigen::Vector3d(0, 1, 0), -xrel / 180.0);
			scn->RotateInSystem(scn->MakeTransd().block<3, 3>(0, 0), Eigen::Vector3d(1, 0, 0), -yrel / 180.0);
			//scn->RotateInSystem(scn->MakeTransd() * scn->CalcParentsTrans(scn->selected_data_index), Eigen::Vector3d(0, 1, 0), -xrel / 180.0);
			//scn->RotateInSystem(scn->MakeTransd() * scn->CalcParentsTrans(scn->selected_data_index), Eigen::Vector3d(1, 0, 0), -yrel / 180.0);
		}
	}
}
