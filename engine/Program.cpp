...
Program::Program(const string &fileName, bool overlay, bool warnings) :
        Program(make_shared<Shader>(GL_VERTEX_SHADER, fileName + ".vs"),
                make_shared<Shader>(GL_FRAGMENT_SHADER, fileName + ".glsl"), overlay, warnings) {}

Program::Program(shared_ptr<const Shader> _vertexShader, shared_ptr<const Shader> _fragmentShader, bool overlay, bool warnings) :
...
void
Program::SetUniform4ui(const string &name, unsigned int v0, unsigned int v1, unsigned int v2, unsigned int v3) const
{
...
int Program::GetUniformLocation(const string &name) const
{
    ...
    if (location == -1 && warnings)
        cerr << "Warning: ignoring uniform '" << name << "' (doesn't exist in program object " << handle << ")" << endl;
